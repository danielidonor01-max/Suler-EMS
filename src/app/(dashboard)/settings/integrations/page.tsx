...
export default function IntegrationsPage() {
  ...
  const toggleIntegration = (id: string) => {
    const integration = settings.integrations.status.find(i => i.id === id);
    ...
    updateSettings(prev => ({
      ...
      integrations: {
        ...prev.integrations,
        status: prev.integrations.status.map(i => 
          i.id === id ? { ...i, connected: !i.connected, lastSync: !i.connected ? 'Just now' : null } : i
        )
      }
    }));

    pushGovernanceActivity(
      `Integration ${!integration.connected ? 'Enabled' : 'Disabled'}`, 
      `The [${integration.name}] connector was ${!integration.connected ? 'activated' : 'deactivated'} by an administrator.`
    );
  };
  ...
  return (
    <RouteGuard allowedRoles={['SUPER_ADMIN']}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {settings.integrations.status.map(integration => {
          const Icon = ICON_MAP[integration.id] || Plug;
          const category = CATEGORY_MAP[integration.id] || 'Other';
          const description = integration.id === 'slack' ? 'Send payroll alerts, leave approvals, and governance notifications to Slack channels.' :
                            integration.id === 'email' ? 'Configure outbound email for system notifications, payslips, and audit alerts.' :
                            integration.id === 'analytics' ? 'Track platform usage patterns and report generation metrics.' :
                            integration.id === 'zapier' ? 'Connect Suler EMS to 5,000+ external apps through automated workflow triggers.' :
                            integration.id === 'hrms' ? 'Bi-directional sync with external HR management systems via REST API.' :
          ...
          return (
            <div key={integration.id} className={`bg-white rounded-[24px] border p-6 shadow-sm space-y-4 transition-all ${integration.connected ? 'border-slate-200' : 'border-slate-100'}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`text-[9px] font-bold uppercase tracking-widest border rounded-full px-2 py-0.5 ${categoryColors[category] || 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                      {category}
                    </span>
                  </div>
                </div>
              ...
              <p className="text-[12px] font-medium text-slate-400 leading-relaxed">{description}</p>
