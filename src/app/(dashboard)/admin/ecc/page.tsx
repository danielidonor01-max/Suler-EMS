...
// Recharts is ~150KB gzipped. Dynamic-import the chart components so the
// library only loads when this page actually renders — keeps it out of
// any shared bundle. ssr:false because ResponsiveContainer measures DOM
// at mount.
const EccTrustRadar = dynamic<{ data: Array<{ subject: string; value: number }> }>(
  () => import('@/components/analytics/EccCharts').then(m => m.EccTrustRadar),
  {
    ssr: false,
    loading: () => (
...
const EccEngagementChart = dynamic<{ data: Array<{ date: string; engagement: number; productivity: number }> }>(
  () => import('@/components/analytics/EccCharts').then(m => m.EccEngagementChart),
  {
    ssr: false,
    loading: () => (
      <div className="h-[400px] w-full flex items-center justify-center text-[12px] text-slate-400">
        Loading chart…
      </div>
    ),
  },
        ...
        <div className="bg-white rounded-[24px] p-10 border border-slate-200 shadow-premium relative overflow-hidden">
           <div className="relative z-10">
              ...
              <div className="h-[300px] w-full mt-4">
                 <EccTrustRadar data={dynamicRadarData} />
              </div>
