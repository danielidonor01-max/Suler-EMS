      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 px-2">
        <div className="space-y-4">
          <div className="flex items-center gap-2.5">
             ...
             <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Sync: Nominal</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-900">Strategic Broadcasts</h2>
            </div>
            <button className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-900 transition-colors">Registry View</button>
          </div>

          <div className="space-y-4">
            {MOCK_BROADCASTS.map((item) => (
              <div key={item.id} className="group bg-white p-6 rounded-[24px] border border-slate-200/60 hover:border-slate-400 transition-all shadow-sm flex items-start justify-between gap-6 cursor-pointer">
                <div className="flex gap-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <span className="px-2.5 py-1 rounded-lg bg-slate-900 text-white text-[8px] font-black uppercase tracking-[0.15em]">{item.category}</span>
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-3 px-2">
            ...
            <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-900">Active Threads</h2>
          </div>

          <div className="space-y-3">
             {MOCK_THREADS.map((thread) => (
               <div key={thread.id} className="bg-white p-5 rounded-[20px] border border-slate-200/60 hover:shadow-md transition-all cursor-pointer group">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                       <div className={`w-1.5 h-1.5 rounded-full ${thread.status === 'ACTIVE' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                       <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{thread.status}</span>
                    </div>
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-slate-50 pt-4">
                     <div className="flex -space-x-2">
                        {[1,2,3].map(i => (
                          <div key={i} className="w-7 h-7 rounded-lg border-2 border-white bg-slate-50 flex items-center justify-center text-[8px] font-black text-slate-400 shadow-sm">
                             {i}
                          </div>
                        ))}
                     </div>
             ...
             <button className="w-full py-4 rounded-[20px] border border-dashed border-slate-200 text-slate-400 text-[11px] font-black uppercase tracking-widest hover:border-slate-400 hover:text-slate-900 hover:bg-white transition-all flex items-center justify-center gap-2.5">
