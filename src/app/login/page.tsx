...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f9fafb] selection:bg-indigo-100 selection:text-indigo-900 font-sans">
      {/* Left rail: brand panel — same dark fill as the in-app sidebar.
          Stacks above the form on small screens, sits left on md+. */}
      <aside className="bg-slate-900 text-white md:w-[44%] lg:w-[40%] xl:w-[36%] flex flex-col items-center justify-center px-8 py-14 md:py-12 relative overflow-hidden">
        {/* Subtle ambient highlights — matches dashboard mood */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -top-[20%] -left-[20%] w-[70%] h-[70%] bg-indigo-500/10 rounded-full blur-[120px]" />
          <div className="absolute -bottom-[25%] -right-[20%] w-[70%] h-[70%] bg-blue-500/10 rounded-full blur-[120px]" />
        </div>
        ...
        <motion.div
          ...
          className="relative z-10 flex flex-col items-center text-center max-w-[420px]"
        >
          <div className="w-16 h-16 rounded-[20px] bg-white/5 backdrop-blur ring-1 ring-white/10 flex items-center justify-center mb-8">
            <div className="w-11 h-11 rounded-[14px] bg-indigo-600 flex items-center justify-center text-white shadow-lg shadow-indigo-900/40">
              <ShieldCheck className="w-6 h-6" />
            </div>
          </div>

          <h1 className="text-[28px] md:text-[32px] font-bold tracking-tight leading-tight">
            Suler Employee Management System
          </h1>
          <p className="text-slate-400 text-[11px] font-bold mt-4 uppercase tracking-[0.25em]">
            Operational Workspace
          </p>

          <p className="text-slate-300 text-[13px] leading-relaxed mt-8 max-w-[340px]">
            Workforce, payroll, finance, leave, and governance — unified for Nigerian enterprises.
          </p>

          <div className="mt-12 flex items-center gap-6 opacity-60">
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-3.5 h-3.5 text-slate-500" />
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">ISO 27001</span>
            </div>
            <div className="w-1 h-1 rounded-full bg-slate-600" />
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">SOC 2</span>
            </div>
          </div>
        </motion.div>
      </aside>

      {/* Right rail: form. min-h-screen on its own so it scrolls if content
          exceeds the viewport (long form + Google + dev creds). */}
      <main className="flex-1 flex items-center justify-center px-6 py-12 md:py-16 overflow-y-auto">
        <motion.div
